...
export default {
  ...
  created() {
    if (this.type === "modify") {
      http.get(`/qna/question/${this.$route.params.no}`).then(({ data }) => {
        ...
        this.question.no = data.no;
        this.question.userid = data.userid;
  ...
  methods: {
    ...
    registArticle() {
      const API_URL = `http://localhost/qna/question`;
      axios({
        url: API_URL,
        method: "POST",
    ...
    modifyArticle() {
      const API_URL = `http://localhost/qna/question/${this.question.no}`;
      axios({
        url: API_URL,
        method: "PUT",
        params: this.question,
